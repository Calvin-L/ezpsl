...
_line_00006(self) ==
  ...
  /\ LET _tmp_4 == self IN
    /\ LET _tmp_5 == ~x IN
      /\ LET _tmp_6 == [_pc EXCEPT ![self] = (SubSeq(_pc[self], 1, (Len(_pc[self]) - 1)) \o <<"_main">>)] IN
        /\ _actor' = _tmp_4
        /\ _pc' = _tmp_6
        /\ x' = _tmp_5
        /\ UNCHANGED _frames
...
_line_00008(self) ==
  ...
  /\ ((_actor = _Undefined) \/ (_actor = self))
  /\ LET _tmp_7 == self IN
    /\ LET _tmp_8 == [_pc EXCEPT ![self] = (SubSeq(_pc[self], 1, (Len(_pc[self]) - 1)) \o <<"_line_00004">>)] IN
      /\ _actor' = _tmp_7
      /\ _pc' = _tmp_8
      /\ UNCHANGED _frames
      /\ UNCHANGED _ret
...
_main(self) ==
  ...
  /\ ((_actor = _Undefined) \/ (_actor = self))
  /\ LET _tmp_9 == self IN
    /\ LET _tmp_10 == [_pc EXCEPT ![self] = (SubSeq(_pc[self], 1, (Len(_pc[self]) - 1)) \o <<(IF TRUE THEN "_line_00006" ELSE "_line_00008")>>)] IN
      /\ _actor' = _tmp_9
      /\ _pc' = _tmp_10
      /\ UNCHANGED _frames
      /\ UNCHANGED _ret
...
NoAssertionFailures == \A self \in UNION {main_calls}:
    /\ (((_pc[self] /= <<>>) /\ (_pc[self][Len(_pc[self])] = "_line_00008")) => FALSE)
\* /include While.ezs

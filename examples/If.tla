...
_line_00005(self) ==
  ...
  /\ (_actor = self)
  /\ LET _tmp_8 == [_pc EXCEPT ![self] = (SubSeq(_pc[self], 1, (Len(_pc[self]) - 1)) \o <<(IF (_globalsScratch["x"] = 0) THEN "_line_00006" ELSE "_line_00008")>>)] IN
    /\ _pc' = _tmp_8
    /\ UNCHANGED _actor
    /\ UNCHANGED _frames
  ...
  /\ \E _tmp_9 \in {1}:
    /\ LET _tmp_10 == _tmp_9 IN
      /\ LET _tmp_11 == [_globalsScratch EXCEPT !["x"] = _tmp_10] IN
        /\ TRUE
        /\ LET _tmp_12 == [_pc EXCEPT ![self] = (SubSeq(_pc[self], 1, (Len(_pc[self]) - 1)) \o <<"_line_00010">>)] IN
...
NoAssertionFailures == \A self \in UNION {main_calls}:
    /\ (_actor = self) => ((((_pc[self] /= <<>>) /\ (_pc[self][Len(_pc[self])] = "_line_00010")) => FALSE))
\* /include If.ezs
